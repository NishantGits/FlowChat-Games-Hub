...
import { GameObject, ObjectType, LANE_WIDTH, SPAWN_DISTANCE, REMOVE_DISTANCE, GameStatus, FLOWCHAT_COLORS } from '../../types';
...
const ParticleSystem: React.FC = () => {
    ...
    useFrame((state, delta) => {
        if (!mesh.current || useStore.getState().status === GameStatus.PAUSED) return;
    ...
    if (furthestZ > -SPAWN_DISTANCE) {
         ...
         if (isLetterDue) {
             ...
             if (availableIndices.length > 0) {
                 ...
                 const color = FLOWCHAT_COLORS[chosenIndex];
