...
import { LANE_WIDTH, GameStatus, PLAYER_SKINS } from '../../types';
...
export const Player: React.FC = () => {
  ...
  const { status, laneCount, takeDamage, hasDoubleJump, activateImmortality, isImmortalityActive, currentSkin, togglePause } = useStore();
  ...
  const { armorMaterial, jointMaterial, glowMaterial, shadowMaterial } = useMemo(() => {
      const skin = PLAYER_SKINS[currentSkin];
      // Immortality: White/Cyan hyper-glow instead of gold to avoid clashing with Gold Prime
      const armorColor = isImmortalityActive ? '#ffffff' : skin.primary;
      const glowColor = isImmortalityActive ? '#00ffff' : skin.secondary;
      ...
      return {
          armorMaterial: new THREE.MeshStandardMaterial({ 
              ...
              metalness: 0.9,
              emissive: armorColor,
              emissiveIntensity: isImmortalityActive ? 0.5 : 0.1
          }),
          jointMaterial: new THREE.MeshStandardMaterial({ color: '#0a0a0a', roughness: 0.8, metalness: 0.2 }),
  ...
  }, [isImmortalityActive, currentSkin]); // Recreate if immortality or skin changes
